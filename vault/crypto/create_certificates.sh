...
openssl genrsa -aes256 \
      ...
      -out ${CA_DIR}/private/vault.key.pem 2048

openssl rsa -in ${CA_DIR}/private/vault.key.pem \
      -out ${CA_DIR}/private/vault.decrypted.key.pem \
      -passin pass:changeit

chmod 400 ${CA_DIR}/private/vault.key.pem
chmod 400 ${CA_DIR}/private/vault.decrypted.key.pem

echo "[INFO] Generating server certificate request"
openssl req -config <(cat ${DIR}/openssl.cnf \
        <(printf "\n[SAN]\nsubjectAltName=DNS:vault")) \
      -reqexts SAN \
      -key ${CA_DIR}/private/vault.key.pem \
      -passin pass:changeit \
      -new -sha256 -out ${CA_DIR}/csr/vault.csr.pem \
      -subj "/C=NN/ST=Unknown/L=Unknown/O=spring-cloud-vault-config/CN=vault"
...
openssl ca -config ${DIR}/openssl.cnf \
      ...
      -passin pass:changeit \
      -batch \
      -in ${CA_DIR}/csr/vault.csr.pem \
      -out ${CA_DIR}/certs/vault.cert.pem
